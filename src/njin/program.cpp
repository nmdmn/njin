...
auto program::on() const -> void { glUseProgram(id_); }

auto program::off() const -> void { glUseProgram(0); }
...
auto program::compile_shader(const string &shader_source, const GLenum &shader_type) const -> GLuint {
...
auto program::to_string() const -> string {
	...
	for (GLint index = 0; index < count; index++) {
		...
		msg_stream << "\t id=" << ::to_string(index) << ", type=" << ::to_string(type) << ", name=\"" << name << "\""
							 << endl;
	}
