...
public:
  ...
  template <typename T> friend auto operator<<(T &stream, const program &program) -> T & {
...
private:
  ...
  auto compile_shader(const std::string &shader_source, const GLenum &shader_type) const -> GLuint;
